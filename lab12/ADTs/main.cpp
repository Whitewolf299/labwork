...
{
	ListNode<int>* prev = front;
	ListNode<int>* current = prev->getNext();
	unordered_map<int, int> have_found;
	have_found[front->getValue()]++;

	while (current != nullptr)
	{
		if (current->getNext() == nullptr)
		{
			if (have_found[current->getValue()] > 0)
			{
				current = nullptr;
